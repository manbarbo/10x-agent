...
/** Contexto de observabilidad Langfuse (sin secretos ni PII). */
export interface AgentLangfuseContext {
  tags?: string[];
...
export interface AgentInput {
  ...
  /** Tags y metadata por canal / feature (sesión y usuario van en el handler). */
  langfuse?: AgentLangfuseContext;
}
...
function buildConfirmationMessage(
  ...
  switch (toolId) {
    ...
    case "calendar_create_event": {
      ...
      let when: string;
      try {
        when = new Date(startStr).toLocaleString("es", {
          dateStyle: "full",
          timeStyle: "short",
        });
      } catch {
      ...
      const emails =
        (args.attendees as string[] | undefined)?.filter(Boolean) ?? [];
      const invite =
    ...
    case "calendar_reschedule_event": {
      ...
      let when: string;
      try {
        when = new Date(ns).toLocaleString("es", {
          dateStyle: "full",
          timeStyle: "short",
        });
      } catch {
    ...
    case "schedule_task": {
      const schedType =
        args.schedule_type === "recurring" ? "recurrente" : "una sola vez";
      const when =
...
export async function runAgent(input: AgentInput): Promise<AgentOutput> {
  ...
  const langfuseHandler = createLangfuseCallbackHandler({
    userId,
    sessionId,
    tags: langfuseCtx?.tags,
    traceMetadata: {
      ...(langfuseCtx?.traceMetadata ?? {}),
      bypass_confirmation: bypassConfirmation,
      ...(resumeDecision != null
        ? { hitl_resume: true, resume_decision: resumeDecision }
        : {}),
    },
  });
  ...
  try {
    ...
    const modelWithTools =
      lcTools.length > 0 ? model.bindTools(lcTools) : model;
    ...
    async function toolExecutorNode(
      ...
      for (const tc of lastMsg.tool_calls) {
        ...
        if (def && toolRequiresConfirmation(toolId)) {
          ...
          try {
            const result = await confirmedHandler(
              tc.args as Record<string, unknown>,
              toolCtx,
            );
            await updateToolCallStatus(db, record.id, "executed", result);
